...
typedef struct{
	char pro_id[20];
	int ar,bu,st,ft,wt,tat,pr;
}pro;

void main()
{
	int i,n,k=0,t,j,m=0,u,v,co,w=0,d,l;
	float ta,avg;
	...
	for(i=0;i<(n-1);i++)
	{
		if(t==pro1[(i+1)].ar)
		{
			for(j=(i+1);j<n;j++)
			{
				if((pro1[j].ar==pro1[i].ar)&&(pro1[i].pr>pro1[j].pr))
				{
					min=pro1[j];
					pro1[j]=pro1[i];
					pro1[i]=min;
	...
	while(i<n)
	{
		for(j=m;j<k;j++)
		{
			if((pro1[i].ar<=(pro2[j].bu+w)) && (pro1[i].pr<=pro2[j].pr))
			{
				m=k-1;
				while(m>j)
				{
				...
				pro2[(k+1)].pr=pro2[(m-1)].pr;
				k=k+2;
				for(u=m+1;u<(k-1);u++)
				{
					for(v=u+1;v<k;v++)
					{
						if(pro2[u].pr>pro2[v].pr)
						{
							min=pro2[u];
							pro2[u]=pro2[v];
							pro2[v]=min;
	...
	if((pro2[0].pr>pro2[1].pr)&&(pro2[0].bu>pro2[1].ar))
	{
		pro2[0].st=0;
		pro2[0].ft=pro2[1].ar;
	}
	...
	{
			pro2[i].st=co;
			if((pro2[i-1].pr>pro2[i].pr)&&(pro2[i-1].bu>pro2[i].ar))
			{
				if((pro2[i].bu>pro2[i+1].ar)&&(pro2[i].pr>pro2[i+1].pr))
				{
	...
	pro2[i].st=pro2[i-1].ft;
	pro2[i].ft=pro2[i].st+pro2[i].bu;
	for(i=0;i<k;i++)
	{
		co=0;
		u=-1;
	...
	for(i=0;i<k;i++)
	{
		...
		if(co==(k-(i+1))) pro2[i].tat=pro2[i].ft-pro2[i].ar;
		else pro2[j].tat=0;
	}
	printf("\nTabular output\n\n");
